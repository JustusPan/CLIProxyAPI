...
func TestOpenAICompatExecutorPreservesPromptCacheKeyForResponsesSource(t *testing.T) {
	var gotPath string
	var gotBody []byte
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		gotPath = r.URL.Path
	...
	payload := []byte(`{"model":"qwen3.6-plus","prompt_cache_key":"pcache_qwen_123","input":[{"type":"message","role":"user","content":"hi"}]}`)
	_, err := executor.Execute(context.Background(), auth, cliproxyexecutor.Request{
		Model:   "qwen3.6-plus",
		Payload: payload,
	}, cliproxyexecutor.Options{
	...
	if got := gjson.GetBytes(gotBody, "prompt_cache_key").String(); got != "pcache_qwen_123" {
		t.Fatalf("prompt_cache_key = %q, want %q; body=%s", got, "pcache_qwen_123", string(gotBody))
	}
}
